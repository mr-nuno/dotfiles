# Dockerfile — .NET Web API, SINGLE-PROJECT layout (multi-stage, Alpine, culture-aware, non-root).
# For a small self-contained service: one csproj holds everything (Endpoints/, Application/,
# Domain/, Infrastructure/ as FOLDERS — same namespaces as the 4-project layout, just one assembly).
# See the "Solution layout variants" section in .claude/docs/folder-structure.md.
#
...
# Replace {Api} with your single project's name.

FROM mcr.microsoft.com/dotnet/sdk:10.0-alpine AS build
WORKDIR /src

# --- Restore layer: copy solution/props + the single csproj first, so `restore` is cached
#     until dependencies change. (Directory.Packages.props is REQUIRED by Central Package
#     Management — restore fails without it.) One project → one COPY line.
